...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      fileInput('syllabus', 'Upload syllabus File (.txt)', accept = ".txt", multiple = FALSE),
      uiOutput("syllabus_ui"),
      fileInput(
    ...
    mainPanel(
      downloadButton("download", "Download Similirity Scoring Result", style="color: #fff; background-color: #337ab7; border-color: #2e6da4"),
...
scoreResult = function(input1, resume_f) {
  case <- read.csv(input1, stringsAsFactors = F)
  # resume_f <- read_file(input2)
  
  # make resume content a dataframe
  resume_fdf <- tibble(title = "User", description = resume_f, ISBN10 = "1538728621", 
                       ISBN13 = "978-1538728628", publisher="max")
  ...
  case_syllabus_result <- case_resume %>%
    ...
    filter(title != "User") %>%
    select(c(title, similarity_score, ISBN10, ISBN13, publisher))
...
server<-function(input , output, session) {
    ...
    output$syllabus_ui <- renderUI({
        textAreaInput('syllabus_content','Syllabus Content', value = content(), height = "300px")
    })
    
    
    content <- reactive({
        
        if (is.null(input$syllabus)) {
            ""
        } else {
            req(input$syllabus)
            
            read_file(input$syllabus$datapath)
        }
    })
